...
public class EV3CommandAdapter extends Operator implements Parameterizable, ISharableAdapter {
    ...
    private Schema BLOCKING_STATUS = new Schema("", FIELD_SUCCESS, FIELD_SUCCESS_POSE, FIELD_COMMAND_TARGET, FIELD_COMMAND_POSE, FIELD_COMMAND_TIME,
                                                FIELD_COMMAND_RATE);
    ...
    public void typecheck() throws TypecheckException {
        ...
        if (getInputSchema(1) == null || !getInputSchema(1).hasField(FIELD_COMMAND_POSE.getName()) || !getInputSchema(1).hasField(FIELD_COMMAND_TIME.getName())
        ...
        if (getInputSchema(2) == null || !getInputSchema(2).hasField(FIELD_LED_ON.getName())
                || getInputSchema(2).getField(getInputSchema(2).getFieldIndex(FIELD_LED_ON.getName())).getElementType() != FIELD_LED_ON.getElementType()) {
    ...
    public void processTuple(int inputPort, Tuple tuple) throws StreamBaseException {
        ...
        if (inputPort == 1) {
            ...
            if (tuple.getSchema().hasField(FIELD_COMMAND_POSE.getName()) && !tuple.isNull(FIELD_COMMAND_POSE.getName())) {
                if (tuple.getSchema().hasField(FIELD_COMMAND_TIME.getName()) && !tuple.isNull(FIELD_COMMAND_TIME.getName())) {
                ...
                } else if (tuple.getSchema().hasField(FIELD_COMMAND_RATE.getName()) && !tuple.isNull(FIELD_COMMAND_RATE.getName())) {
                    int pose = tuple.getInt(FIELD_COMMAND_POSE.getName());
                ...
                if (tuple.getSchema().hasField(FIELD_COMMAND_TIME.getName()) && !tuple.isNull(FIELD_COMMAND_TIME.getName())
                        && tuple.getSchema().hasField(FIELD_COMMAND_RATE.getName()) && !tuple.isNull(FIELD_COMMAND_RATE.getName())) {
                    int rate = tuple.getInt(FIELD_COMMAND_RATE.getName());
                    int time = tuple.getInt(FIELD_COMMAND_TIME.getName());
                    // try running the motor & report on the result
    ...
    public boolean blockingPoseTime(int p, int t, String port) {
        ...
        byte sensorPort = connectTo.getSensorPortByte(port);
        // get starting pose.
        int pStart = (int) connectTo.robot.getSensor().getValueSI(sensorPort, Sensor.TYPE_LARGE_MOTOR, Sensor.LARGE_MOTOR_DEGREE);
        ...
        // error
        int pFinish = (int) connectTo.robot.getSensor().getValueSI(sensorPort, Sensor.TYPE_LARGE_MOTOR, Sensor.LARGE_MOTOR_DEGREE);
        if (r > 100 || r < -100) {
    ...
    public void reportBlockingStatus(boolean b, Tuple t) throws StreamBaseException {
        ...
        out.setInt(FIELD_SUCCESS_POSE.getName(), (int) connectTo.robot.getSensor().getValueSI(sensorPort, Sensor.TYPE_LARGE_MOTOR, Sensor.LARGE_MOTOR_DEGREE));
