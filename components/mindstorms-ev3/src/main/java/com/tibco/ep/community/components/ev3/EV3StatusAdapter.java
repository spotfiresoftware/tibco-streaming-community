...
/**
 * EV3 "input" adapter object. This adapter allows for monitoring the values of the motors and sensors connected to the
...
public class EV3StatusAdapter extends Operator implements Parameterizable, ISharableAdapter, Runnable {
    ...
    public EV3StatusAdapter() {
        ...
        setDisplayDescription("The EV3 Status Adapter allows you request sensor readings from the EV3 Brick with a StreamBase module.");
    ...
    public void typecheck() throws TypecheckException {
        ...
        if (getInputSchema(0) == null || !getInputSchema(0).hasField(FIELD_TARGET_PORT.getName())) {
            throw new TypecheckException(String.format("The control port schema must at least have a field named %s of type String",
    ...
    public RobotPort initBotPort(String name) {
        ...
        return new RobotPort(name, getPortStreaming(name), connectTo.getSensorPortByte(name), getSchemaForSensorType(type), type);
    }
     ...
     * Input: Control
     * 
     * Output: -ButtonOutput -MotorOutput -TouchOutput -ColorOutput -UltraOutput -GyroOutput -IROutput
     */
    ...
    public Schema createMotorOutputSchema() {
        ...
        Schema motorSchema = new Schema("motorSchema", new Schema.Field(FIELD_DEGREES, returnType), new Schema.Field(FIELD_ROTATION, returnType),
    ...
    public Schema createTouchOutputSchema() {
        CompleteDataType returnType = OutputType == OutputTypeEnum.PERCENT ? CompleteDataType.forInt() : CompleteDataType.forDouble();
        // if a percentage is requested, it will be an integer; otherwise a double
        Schema touchSchema = new Schema(SensorTypeEnum.TOUCH.toString(), new Schema.Field(FIELD_TOUCH, returnType), new Schema.Field(FIELD_BUMPED, returnType));
        return touchSchema;
    }
    ...
    public Schema createColorOutputSchema() {
        CompleteDataType returnType = OutputType == OutputTypeEnum.PERCENT ? CompleteDataType.forInt() : CompleteDataType.forDouble();
        // if a percentage is requested, it will be an integer; otherwise a double
        Schema colorSchema = new Schema(SensorTypeEnum.COLOR.toString(), new Schema.Field(FIELD_COLOR, CompleteDataType.forString()),
    ...
    public Schema createUltraOutputSchema() {
        ...
        Schema ultraSchema = new Schema(SensorTypeEnum.ULTRA.toString(), new Schema.Field(FIELD_DIST_CM, returnType),
                                        new Schema.Field(FIELD_DIST_IN, returnType), new Schema.Field(FIELD_LISTEN, CompleteDataType.forBoolean()));
        return ultraSchema;
    }
    ...
    public Schema createGyroOutputSchema() {
        CompleteDataType returnType = OutputType == OutputTypeEnum.PERCENT ? CompleteDataType.forInt() : CompleteDataType.forDouble();
        // if a percentage is requested, it will be an integer; otherwise a double
        Schema gyroSchema = new Schema(SensorTypeEnum.GYRO.toString(), new Schema.Field(FIELD_ANGLE, returnType), new Schema.Field(FIELD_RATE, returnType));
        return gyroSchema;
    }
    ...
    public Schema createIROutputSchema() {
        CompleteDataType returnType = OutputType == OutputTypeEnum.PERCENT ? CompleteDataType.forInt() : CompleteDataType.forDouble();
        // if a percentage is requested, it will be an integer; otherwise a double
        Schema IRSchema = new Schema(SensorTypeEnum.IR.toString(), new Schema.Field(FIELD_PROXIMITY, returnType), new Schema.Field(FIELD_REMOTE, returnType));
        return IRSchema;
    }

    /***************************************************************************************
     * The getter and setter methods provided by the Parameterizable object. * StreamBase Studio uses them to determine
     * the name and type of each property * and obviously, to set and get the property values. *
     ***************************************************************************************/
